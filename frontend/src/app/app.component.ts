...
export class AppComponent implements OnInit {
  ...
  fetchPosts(): void {
    this.http.get<any[]>('http://34.118.47.183:5001/api/posts')
      .subscribe({
        next: (data) => {
          this.posts = data;
  ...
  onSearch(): void {
    ...
    this.http.post('http://34.118.47.183:5001/api/fetch_posts', requestBody)
      .subscribe({
  ...
  onDownloadCSV(): void {
    this.http.get('http://34.118.47.183:5001/api/get_report', { responseType: 'blob' })
      .subscribe({
        next: (blob) => {
  ...
  onClearDB(): void {
    this.http.post('http://34.118.47.183:5001/api/clear_db', {})
      .subscribe({
        next: (response) => {
